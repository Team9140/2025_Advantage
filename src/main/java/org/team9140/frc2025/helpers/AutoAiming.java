...
public class AutoAiming {
    ...
    public enum ReefFaces {
        ...
        private enum Sides {
            ...
            Sides(Angle angleToCenter) {
                ...
                this.offset =
                        new Transform2d(
                                new Translation2d(
                                        AutoAlign.REEF_RADIUS.in(Meters),
                                        new Rotation2d(angleToCenter)),
                                this.direction);
            }
        ...
        ReefFaces(
                Pose2d center, Sides face, boolean isFar, int tagId, Distance algaeElevatorHeight) {
            this.isFar = isFar;
            this.tagId = tagId;
        ...
        public Pose2d getLeft(ElevatorSetbacks height) {
            return this.pose
                    .plus(
                            AutoAlign.HORIZONTAL_BRANCH_DISTANCE_FROM_CENTER.times(
                                    this.isFar ? -1 : 1))
        ...
        if (pose.getX() < 17.548225 / 2) {
            double angle =
                    MathUtil.inputModulus(
                            pose.minus(FieldItemPoses.REEF_BLUE.getTranslation())
                                            .getAngle()
                                            .getDegrees()
                                    + 30,
                            0,
            ...
            double angle =
                    MathUtil.inputModulus(
                            pose.minus(FieldItemPoses.REEF_RED.getTranslation())
                                            .getAngle()
                                            .getDegrees()
                                    + 30,
                            0,
