...
public class Elevator extends SubsystemBase {
  ...
  public void periodic() {
    ...
    if (setpoint.position < 0.0 || setpoint.position > Constants.Elevator.MAX_HEIGHT.in(Meters)) {
      setpoint =
          new State(
    ...
    return this.runOnce(
        () -> {
          ...
          this.goalState =
              new State(
                  MathUtil.clamp(
                      goal.get().in(Meters), 0.0, Constants.Elevator.MAX_HEIGHT.in(Meters)),
                  0.0);
  ...
  public final Trigger atPosition =
      new Trigger(
          () -> this.getPosition().isNear(this.goal.get(), Constants.Elevator.POSITION_epsilon));
  public final Trigger isStowed =
      new Trigger(() -> this.getPosition().isNear(Constants.Elevator.STOW_height, Inches.of(0.75)));
}
