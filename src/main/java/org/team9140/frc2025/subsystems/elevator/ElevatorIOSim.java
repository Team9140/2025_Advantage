...
public class ElevatorIOSim implements ElevatorIO {
  ...
  public static final Matrix<N2, N2> A =
      MatBuilder.fill(
          ...
          1,
          0,
          -gearbox.KtNMPerAmp // Torque per Amp of input
              / (gearbox.rOhms
    ...
    if (closedLoop) {
      ...
      for (int i = 0; i < Constants.LOOP_PERIOD.in(Seconds) / (1.0 / 1000.0); i++) {
        setInputTorqueCurrent(
            controller.calculate(simState.get(0) / Constants.Elevator.SPOOL_RADIUS.in(Meters))
                + feedforward);
        update(1.0 / 1000.0);
      }
    } else {
  ...
  private void setInputVoltage(double voltage) {
    setInputTorqueCurrent(
        gearbox.getCurrent(simState.get(1) / Constants.Elevator.SPOOL_RADIUS.in(Meters), voltage));
  }
                    ...
                    .plus(
                        VecBuilder.fill(
                            ...
                            -Constants.G.in(MetersPerSecondPerSecond)
                                * Math.sin(Constants.Elevator.ElevatorAngle.in(Radians)))),
            simState,
